...
function waldoClicked() {
    ...
    // record to stats table
    $('#stats').append(`Round ${timeTable.length} - finished in ${displayTime(timeTable[timeTable.length - 1])}`);
...
function draw() {
    ...
    for (var row = 0; row < 20; row++) {
        ...
        for (var col = 0; col < 20; col++) {
            ...
            code += `<div id="character-box" class="col bg-${waldo} show-waldo m-0 p-0" style="background-image: url(${
                isWaldo ? characters.waldo : characters.potato
            });"></div>`;
        }
